...
2: graphics are special, showing creativity, an original design, and/or exceptional attention to detail; analysis is exceptionally thorough and well thought 
...
library(leaflet)
```
Code for Location of Parking Lot 
...
base_plot
```
Code for Location of Group Campsite 
```{r, message = FALSE, warning = FALSE}
...
base_plot
```
Code for Location of Remote Campsite
```{r, message = FALSE, warning = FALSE}
...
base_plot
```
Code for Plotting of all 3 sites 
```{r, message = FALSE, warning = FALSE}
base_plot <- leaflet() %>%
  addTiles() %>%
  addMarkers(lng = ~lon, lat = ~lat, data = group_campsite,
...
```
Code for Locations of Streams and Wetlands:
```{r, message = FALSE, warning = FALSE}
leaflet() %>%
addTiles %>% 
...
```
Code for Contours:
We chose to use the 3m contour dataset because much more detailed compared to the 30 ft dataset.
```{r,message = FALSE, warning = FALSE}
leaflet() %>%
addTiles() %>%
addPolylines(data = pluck(macleish_layers, "contours_3m"), weight = 1, color = "red") 
```
Code for Adding contours to parking, campsites, streams/wetlands, and contour maps:
```{r, message = FALSE, warning = FALSE}
leaflet() %>%
addTiles() %>%
...
addPolylines(data = pluck(macleish_layers, "contours_3m"), weight = 1, color = "red")
```
Code for Location of Challenge Courses
```{r, echo = FALSE}
